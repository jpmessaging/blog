...
原文記事の更新検出と既存抄訳記事の更新は、[既存抄訳記事の更新対応自動化](./translation-update-automation.md) を参照してください。
...
- `title`、`date`、`tags` が正しい形式で存在すること。`date` は `YYYY-MM-DD HH:mm` を新規記事の標準とし、既存記事との互換性のため `YYYY-MM-DD` と秒まで含む `YYYY-MM-DD HH:mm:ss` も許容する
- `lastupdate` がある場合に、日本語記事を更新した日本時間の日付として `YYYY-MM-DD` 形式であること
- 新規記事では `lastupdate` を設定しないこと
...
- 原文記事の更新検出と既存抄訳記事の更新は、[別の仕組み](./translation-update-automation.md) として管理する
