...
2026 年 9 月に AppID 許可リストを構成し、`EWSEnabled = True` に設定することで EWS を有効なままにすることを明示的に選択していない Exchange Online テナントでは、EWS は **既定でブロック** (`EWSEnabled = False`) されます。この時点で、以下の状態になります。
