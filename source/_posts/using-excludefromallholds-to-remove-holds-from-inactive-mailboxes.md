...
ExcludeFromAllHolds パラメーターは、電子情報開示ホールド、訴訟ホールド、および制限付き保持ポリシーを除き、非アクティブなメールボックスに適用されているすべてのホールドを削除するためのコマンドレット スイッチです。このパラメーターは、訴訟ホールドや制限付き保持ポリシーが適用されていない場合に、不要な保持ホールドを削除し、メールボックスを完全に削除できるようにするシナリオを対象としています。ただし、コマンドレットは規制遵守に必要な訴訟ホールドを引き続き保持します。
