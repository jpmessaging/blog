...
この決定は、できるだけ多くのユーザーがスムーズに移行できるよう、複数のライセンスを持つデバイスおよびアプリケーション ベンダーとの広範な協力の末に下されました。ユーザーおよび組織がデバイスとアプリケーションを最新のサポート対象バージョンに更新し続けている場合、サービスの中断は最小限に抑えられるはずです。変更が有効になる前に、すべてのユーザーでデバイスとアプリケーションが最新の状態であるか確認することをお勧めします。
