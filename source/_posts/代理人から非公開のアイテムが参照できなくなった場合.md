...
このエラーメッセージが表示される場合の多くの原因が、複数の代理人がいるシナリオで、代理人設定を付与したまま一部のユーザーが無効化されてしまったときであることを確認しています。  
過去に代理人設定をし、権限を削除せずに無効化したユーザーがいる場合は、無効化したユーザーをいったん再度有効化し、代理人権限を削除してください。  
  
予定表フォルダーなどフォルダーに対して付与した権限は以下のコマンドで削除します。  
```PowerShell  
Remove-MailboxFolderPermission -Identity <ユーザー>:\calendar -User <無効にしたユーザー>
```
※フォルダー名が日本語の場合は、`<ユーザー>:\予定表` と指定します。  
  
代理送信権限は、以下のコマンドで削除し、必要なユーザーにあらためて代理送信権を付与します。  
```powershell
Set-Mailbox <ユーザー> -GrantSendOnBehalfTo $null
```
  
こちらで回避できない場合は、(2) の手順をお試しください。  

### (2) ResetDelegateUserCollection スイッチで代理人設定と隠しルールを削除  
  
管理者から以下のコマンドレット スイッチを実行し、隠しルールをリセットします。  

```powershell
Remove-MailboxFolderPermission -Identity <ユーザー>:\calendar -ResetDelegateUserCollection
```
  
このコマンドは、予定表の参照や書き込みの権限は削除されません。
こちらで回避できない場合は、(3) の手順をお試しください。  
