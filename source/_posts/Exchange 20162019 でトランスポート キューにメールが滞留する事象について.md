...
対処方法としては、上記公開情報に記載の通り 「自動化された対処方法 (Automated Solution)」 と 「手動の対処方法 (Manual Solution)」 の 2 つの方法がございます。それぞれ後述します。
...
A. Exchange 2013 は今回の事象によりサービスがクラッシュしてメールが滞留することはございません。しかし、もし Exchange 2013 側で問題のマルウェア定義ファイル バージョン “2201010001” を取得していた場合は、Exchange 2013 メールボックス サーバー上にて「自動化された対処方法 (Automated Solution)」または「手動の対処方法 (Manual Solution)」の手順を実施ください。対処を実施しない場合、今後最新のマルウェア定義ファイルの取得が行われません。
...
**Q. 「自動化された対処方法 (Automated Solution)」よりスクリプトを実行した際、警告「WARNING: WARNING: Unable to process update request because engine metadata is not available. Attempting to synchronize」が表示されます。**  
A. Exchange サーバーがプロキシ経由で Internet に接続している場合、以下の手順でプロキシを設定します。

- Exchange 管理シェルを起動
- Add-PSSnapin Microsoft.Forefront.Filtering.Management.Powershell を実行
- Set-ProxySettings -Enabled \$true -Server <ProxyServer> -Port <proxy.port> を実行
...
- C:\Windows\System32 より msvcr110.dll をコピーし、以下ディレクトリに配置
