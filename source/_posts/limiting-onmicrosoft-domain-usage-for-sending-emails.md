...
悪用防止とメールの配信性向上のため、ポリシーを変更します。今後、MOERA ドメインはテスト用途のみで使用し、通常のメール送信には使用しないことを推奨します。onmicrosoft.com ドメインから外部宛てに送信するメッセージに対し、組織単位で 24 時間のローリング ウィンドウで最大 100 件の外部受信者数に制限するスロットリングを導入します。受信メールには影響しません。外部受信者数は、配布リストなどの展開後の総数でカウントされます。送信者がこの上限に達した場合、以下の内容を含む NDR (配信不能レポート) が返送されます。
