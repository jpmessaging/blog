---
title: "Exchange Online の ActiveSync における証明書ベースの認証エンド ポイントの変更について"
date: 2025/11/4
...
継続的なセキュリティ強化の一環として、Exchange ActiveSync の証明書ベースの認証（CBA）動作に最近変更が加えられました。この変更により TLS 1.3 がサポートされるようになり、セキュリティと信頼性がさらに向上します。

#### 何が変わるのか？

今回の変更により、すべての Exchange ActiveSync の証明書ベースの認証（CBA）トラフィックは、テナントの場所に応じて新しい専用エンドポイントへルーティングされるようになります。
...
この変更についてご不明点やご懸念がある場合は、ご利用の SEG ベンダーまでお問い合わせください。安全な環境を維持するためのご協力に感謝いたします。
