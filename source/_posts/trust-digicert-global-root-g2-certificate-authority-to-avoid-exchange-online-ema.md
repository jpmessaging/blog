...
ただし、組織が SMTP 経由で Exchange Online とメールの送受信を行い、以下の 2 つの条件のいずれかに該当する場合は、「**何をすべきか**」に記載されているインストール手順を実行する必要がある場合があります。
...
証明書バンドル (**.p7b / PKCS #7** ファイル形式) は、以下からダウンロードできます (最新のダウンロードを使用してください):
...
ダウンロード後、管理者権限で PowerShell を開き、以下のコマンドを実行します。パスとファイル名は、取得した実際の .p7b バンドルに合わせて更新してください:
...
DigiCert Global Root G2 ルート証明書をインストールせず、システムが古いルート証明書に依存して検証を行っている場合、2026 年 3 月 22 日以降、メール フローが中断される可能性があります。発生する具体的な問題は、オペレーティング システムとアプリケーションの構成によって異なります。以下にいくつかの例を示します:
...
- **2026/3/16:** タイムラインを更新し、期限を 3 月 22 日に変更しました。証明書バンドルが再公開されたことを記載しました
- **2026/3/12:** この変更は SMTP 経由でメールを送受信するシステムに適用されることを明記しました
- **2026/2/23:** 中間証明書のインポートと検証に関する詳細を追加しました
