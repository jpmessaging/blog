...
Exchange Online PowerShell の -Credential パラメーターは ROPC に依存しているため、MFA や条件付きアクセスの要件を満たしていません。MFA の強制適用、先進認証に関する原則、ならびに Microsoft の包括的なセキュリティ基準に対応するため、**2026 年 7 月以降** にリリースされる新しい Exchange Online PowerShell バージョンでは、-Credential パラメーターのサポートが廃止される予定です。

`この変更は Connect-ExchangeOnline と Connect-IppsSession の両方のコマンドレットに適用されます。`

タイムライン上は 2026 年 7 月からとなりますが、期限まで待たずに可能な限り早期に -Credential パラメーターの使用から移行することを強く推奨します。
...
#### タイムライン
- **現在の状態:** *-Credential* パラメーターは現時点でもサポートされており、2026 年 7 月までにリリースされるすべてのモジュールで引き続き利用可能です。
...
- **2026 年 7 月以降:** 新たにリリースされる Exchange Online PowerShell モジュールの新しいバージョンでは、-Credential パラメーターはサポートされなくなります。
