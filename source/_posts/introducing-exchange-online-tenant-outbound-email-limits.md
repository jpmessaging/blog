...
<p style="background: #FFFF99; padding: .5em; margin: 1em 0 1em 0;"><strong>2026 年 8 月 13 日更新: </strong>不正使用に対する保護をさらに強化し、Exchange Online リソースの公正な利用を確保するため、一部のテナントに対するテナント外部受信者レート制限 (TERRL) クォータの計算方法を更新します。これらの変更は 2026 年 9 月 14 日からロールアウトを開始します。また、<em>GCC、GCCH、DoD、および 21Vianet 環境への TERRL のロールアウトは延期されました。</em></p>
...
テナント外部受信者レート制限は、**WW 環境のテナントに対して完全に実装されています**。
...
- **試用版テナント**: ライセンス数に関係なく、1 日あたりの外部受信者の合計数は 500 に制限されます。
- **新しいテナントのクォータ**: 新しく作成されたテナントでは、初期オンボーディング期間中、計算された TERRL クォータが一時的に引き下げられます。

| **テナントの経過日数** | **適用されるクォータ** |
| --- | --- |
| 31 日未満 | 標準の計算済みクォータの 10 % (標準クォータの計算方法は以下を参照) |
| 31 日以上 60 日以下 | 標準の計算済みクォータの 25 % |
| 60 日超 | 標準の計算済みクォータの 100 % |

- **無償の EDU ライセンスがあるテナントのクォータ計算式**: 無償の Education ライセンス (Microsoft 365 A1 など) があるテナントでは、購入済みメール ライセンスと無償 EDU ライセンスの両方を考慮するようにクォータの計算方法を更新します。

以下の式を使用します。

    500 * (購入済みメール ライセンス数 ^ 0.7 + 無償 EDU ライセンス数 ^ 0.3) + 9500

更新後の計算方法では、無償 EDU ライセンスもクォータに反映されますが、購入済みメール ライセンスよりも重み付けが小さくなります。

- **その他すべてのテナントに対する標準の計算済みクォータ**: 1 日あたりの外部送信制限は、テナントのメール ライセンス数 (Exchange Online または Exchange Online Protection を含むライセンス) に基づいています。ライセンス数が増えると、1 日に送信できる外部受信者数も増加しますが、ライセンスごとの増加率は減少します。

    500 * (試用版ではないメール ライセンス数 ^ 0.7) + 9500

以下の表に、さまざまなライセンス数を持つテナントのサンプルの制限値を示します。

| **試用版ライセンスおよび EDU ライセンスを除くメール ライセンス数** | **TERRL** |
| --- | --- |
| 1 | 10,000 |
| 2 | 10,312 |
| 10 | 12,006 |
...
管理者が送信メールの量を計画および追跡できるようにするため、Exchange 管理センター (EAC) のレポート (Reports) &gt; メール フロー (Mail flow) &gt; **テナント外部受信者レート (Tenant Outbound External Recipients Rate)** に新しいレポートがあります。このレポートでは、現在の外部受信者の数、テナントの日次クォータ、クォータの使用量、および制限を超えた場合にブロックされた受信者の数が表示されます。また、制限の適用が有効か無効かも表示されます。
...
**最近の変更点**
- 2026/8/13: 記事を大幅に更新し、制限の更新や一部のクラウド環境へのロールアウトの延期などを反映しました。
