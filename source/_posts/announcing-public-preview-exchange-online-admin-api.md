...
現在、API のレスポンスには追加のプロパティが含まれていますが、一般提供 (GA) では各エンドポイントのドキュメントに記載されているプロパティのみが利用可能となります。一般提供時に利用可能なプロパティ一覧は、各エンドポイントのドキュメントをご参照ください。出力プロパティの変更がある場合は、一般提供のリリース時にお知らせします。
