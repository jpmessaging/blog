...
2022 年 11 月以降、protection.microsoft.com の URL は使用できなくなりました。この投稿で言及されている迷惑メール対策機能については、<a href='https://security.microsoft.com/antispam'>security.microsoft.com</a> にアクセスしてください。
</div>
...
- デスクトップ版 Outlook で仕分けルールとして転送のルールを設定できます。ユーザーは Outlook で [ファイル] - [仕分けルールと通知の管理] にて自動転送を設定できます。Outlook on the web (OWA / Ootw) を使用する場合もルールを設定することが可能です。パブリック フォルダーに (パブリック フォルダー アシスタントを使用して) 設定した転送のルールも仕分けルールと同様に動作します。
- デスクトップ版 Outlook で不在時の応答のルールを作成する際に、自動転送を構成することもできます。[ファイル] - [自動応答 (不在時)] - [ルール] - [ルールの追加] - [転送] で設定します。不在時の応答 (OOF) メールは[送信者ごとに 1 回のみ送信される](https://docs.microsoft.com/exchange/troubleshoot/email-delivery/understand-troubleshoot-oof-replies)動作ですが、このルールを設定すると、OOF が有効な間はすべてのメッセージに対してメールが転送されます。
...
- 管理者は Exchange 管理センター (EAC) でメールボックスのプロパティから転送を構成することが可能です。この設定は、EAC のメールボックスのプロパティ内の [メールの転送] で設定できます。メールボックスのプロパティから内部宛ての自動転送を構成すると、メールボックスの **ForwardingAddress** パラメーターが設定されます。一方、メールボックスのプロパティから外部宛ての自動転送を構成すると、メールボックスの **ForwardingSmtpAddress** パラメーターが設定されます。
...
### 迷惑メール対策の送信ポリシーを使用した外部転送の制御

最近リリースされたこの機能は、Microsoft 365 Defender ポータルの迷惑メール対策の送信ポリシー ([こちら](https://security.microsoft.com/antispam)) で利用できます。以下の画面ショットにあるように、3 つのオプションがあります。既定の構成は「自動 - システム制御」です。他のオプションとして「オフ」と「オン」があります。「オフ」は自動転送が無効であることを意味し、「オン」は自動転送が有効であることを意味します。
...
迷惑メール対策の送信ポリシーによって外部への自動転送がブロックされた場合、NDR (配信不能レポート) は転送元のメールボックスではなく、元の送信者に送り返されます。NDR には以下の診断情報が含まれます。
...
- この設定は、Outlook のルールと OWA のオプション (ForwardingSmtpAddress パラメーター) を使用して構成された自動転送をブロックできます。Exchange 管理センターからメールボックスのプロパティに設定した外部への自動転送もブロックします。
...
|自動転送のオプション|リモート ドメイン|トランスポート ルール|迷惑メール対策の送信ポリシー|
|:--|:--|:--|:--|
|Outlook のルールを使用した転送をブロックする|Yes|Yes|Yes|
|Outlook 不在時の応答のルールを使用した転送をブロックする|Yes|Yes|Yes|
|OWA の転送設定 (ForwardingSmtpAddress) を使用した転送をブロックする|Yes|No|Yes|
|管理者が EAC で設定した外部への転送 (ForwardingSmtpAddress) をブロックする|Yes|No|Yes|
|Power Automate / Flow を使用した転送をブロックする|No|Yes|No|
|自動転送がブロックされた際に送信者は NDR を受信するか|No|Yes|Yes|
|カスタマイズときめ細かい制御|No|Yes|Yes|

### 上記の複数の設定で自動転送が制御されている場合にどうなるか？

これらすべての手法がどのように連携するのか、頻繁にご質問をいただいています。上記の方法のいずれかで自動転送がブロックされるものの、別の方法では許可されている場合、どうなるでしょうか？例えば、リモート ドメインもしくはトランスポート ルールでは自動転送がブロックされているものの、迷惑メール対策の送信ポリシーでは許可されている場合、何が起きるでしょうか？その答えは、1 つの設定で制限されていれば、すべての自動転送が制限されます。

例:

- 迷惑メール対策の送信ポリシーで自動転送がオン (許可) に設定されている
...
実現したい内容に応じて、上記の機能を組み合わせてご使用いただけます。1 つのオプションですべてのシナリオに対応できるわけではありません。要件に応じて、本当に必要なのであれば 4 つのオプションをすべて実装できます。例えば、リモート ドメインの設定は受信者のドメインごとの制御であるため、一部のドメインを除いてすべての自動転送を制限したい場合に便利な設定です。一方、迷惑メール対策の送信ポリシーは送信者に応じて制御できます。少数のメールボックス (自動転送を構成する必要のある正当なビジネス要件を持つユーザー) に対してのみ外部への自動転送を許可し、他のすべてのユーザーに対して外部への自動転送をブロックする場合は、迷惑メール対策の送信ポリシーが最も適しています。もしくは、少数のメールボックスと少数の外部ドメインのみに自動転送を許可する場合は、これら 2 つのオプションを組み合わせて使用できます。他に少し複雑な別の例も紹介します。
